...
const styles = tv({
  base: 'absolute peer z-30 [&:first-child+*]:scale-95 [&:first-child+*]:z-20 [&:first-child+*+*]:scale-90 [&:first-child+*+*~*]:scale-50 [&:first-child+*+*~*]:z-10 [&:first-child+*+*]:z-10 top-0 [&:first-child+*]:-translate-y-1 [&:first-child+*+*]:-translate-y-1.5 left-0 right-0 flex shrink-0 duration-300 items-center gap-2 flex-auto pl-3 pr-1 py-1 text-sm border rounded-xl backdrop-blur-xl backdrop-saturate-200 min-h-10 shadow-lg shadow-zinc-800/5 transform transition',
  slots: {
    content: 'flex-auto',
